...
answer1 = sum . map calc
  where calc match@(opponent, player) = score player (resultForMatch match)

answer2 = sum . map calc
  where calc match@(opponent, result) = score (handForCondition match) result
...
debugging x@(opponent, player) = concat $ intersperse " " $
