...
def answer1(data):
    acc = 0
    for i in data:
        acc += len(set.union(*map(set,i)))
    return acc

def answer2(data):
    acc = 0
    for i in data:
        acc += len(set.intersection(*map(set,i)))
    return acc

info = parse(data)
print("Answer 1:", answer1(info))
info = parse(data)
print("Answer 2:", answer2(info))

# there is some weirdness going on here
# it seems that I can't run both answer 1 and answer 2 on the same data
# because they are modifying the data
# these are the times when I miss haskell's purity
