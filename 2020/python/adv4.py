...
predicates = {
        ...
        'hcl': lambda x: x[0] == '#' and len(x) == 7 and all(i in '0123456789abcdef' for i in x[1:]),
...
def is_valid(passport):
    for field, predicate in predicates.items():
        if not predicate(passport[field]):
            return False
    return True

def main():
    info = parse(data)
    correct_fields = list(filter(required_fields_present, info))
    print("Answer 1:", len(correct_fields))
    valid = list(filter(is_valid, correct_fields))
    print("Answer 2:", len(valid))
